...
func InitConfig() {
	...
	configFileName := fmt.Sprintf("10-distributed-lock/02-grpc-redsync/%s-prod.yaml", configFilePrefix)
	if debug {
		configFileName = fmt.Sprintf("10-distributed-lock/02-grpc-redsync/%s-dev.yaml", configFilePrefix)
	}
